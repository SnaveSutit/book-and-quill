...
export namespace CHARSETS {
	export const ALPHA_LOWERCASE = charcodeArray('abcdefghijklmnopqrstuvwxyz')
	export const ALPHA_UPPERCASE = charcodeArray('ABCDEFGHIJKLMNOPQRSTUVWXYZ')
	export const ALPHA = ALPHA_LOWERCASE.concat(ALPHA_UPPERCASE)
	export const NUMBER = charcodeArray('0123456789')
	export const NUMBER_START = NUMBER.concat(charcodeArray('-.'))
	export const INT_START = charcodeArray('-123456789')
	export const ALPHANUMERIC = ALPHA.concat(NUMBER)
	export const VERTICAL_WHITESPACE = charcodeArray('\n\r')
	export const WHITESPACE = charcodeArray(' \t').concat(VERTICAL_WHITESPACE)
	export const LITERAL = ALPHANUMERIC.concat(charcodeArray('._-+'))
	export const LITERAL_START = ALPHA.concat(charcodeArray('_'))
	export const QUOTES = charcodeArray(`"'`)
	export const BINARY = charcodeArray('01')
	export const HEXADECIMAL = NUMBER.concat(charcodeArray('abcdef'), charcodeArray('ABCDEF'))
	export const SYNTAX_BOUNDARY = WHITESPACE.concat(QUOTES, charcodeArray(',:[]{}'))
	export const UNICODE_ESCAPE_SEQUENCE_START = charcodeArray('uUx')
	export const NAMED_UNICODE_ESCAPE_SEQUENCE_START = charcodeArray('N')
}
...
export class TextComponentParser {
	...
	constructor(options?: { minecraftVersion?: string; enabledFeatures?: number }) {
		this.minecraftVersion = options?.minecraftVersion ?? TextComponent.defaultMinecraftVersion

		if (typeof options?.enabledFeatures === 'number') {
			this.enabledFeatures = options.enabledFeatures
		} else {
			this.enabledFeatures = TextComponentParser.defaultFeatures

			switch (true) {
				case compareVersions(this.minecraftVersion, '1.21.9') >= 0:
				...
				case compareVersions(this.minecraftVersion, '1.21.4') >= 0:
					this.enabledFeatures |= FEATURES.SHADOW_COLOR
			}
		}
	}

	parse(input: string): TextElement {
		this.currentNestingDepth = 0
		this.s = new StringStream(input)
		...
		try {
			...
			if (this.s.item) {
				this.throwSyntax(
					`Unexpected trailing ${this.s.item} after end of JSON text component`
				)
			}
			return result
		} catch (e: any) {
			if (e instanceof JsonTextParserError) {
				throw new JsonTextParserError(
					`Internal parser error: ${e.message}\nThis is a bug, please report it to the developer of the book-and-quill library.`
				)
			}
			throw e
		}
	...
	private throwSyntax(message: string, options?: SyntaxErrorOptions): never {
		throw new SyntaxPointerError(message, this.s, options)
	}

	private consumeWhitespace() {
		while (CHARSETS.WHITESPACE.includes(this.s.item)) {
			this.s.advance()
		}
	}

	private assertTextObjectColorIsValid(color: string): asserts color is Color {
		if (color.startsWith('#')) {
	...
	private parseTextElement(): TextElement {
		...
		if (this.s.item === CHARS.LEFT_CURLY_BRACKET) {
			result = this.parseTextObject()
		} else if (this.s.item === CHARS.LEFT_SQUARE_BRACKET) {
			result = this.parseTextElementArray()
		} else if (
			CHARSETS.QUOTES.includes(this.s.item) ||
			CHARSETS.LITERAL_START.includes(this.s.item)
		) {
			result = this.parseTextPrimitive()
		} else {
			this.throwSyntax(
				`Unexpected ${this.s.item ? `'${String.fromCharCode(this.s.item)}'` : 'end of input'} while parsing JSON text component`
			)
		}
	...
	private expect(
		...
		if (
			...
			if (Array.isArray(toBe)) {
				this.throwSyntax(
					`Expected one of '${toBe.map(c => String.fromCharCode(c)).join("', '")}' ${message}. Found '${thing !== undefined ? String.fromCharCode(thing) : 'undefined'}' instead`,
				)
			}
			this.throwSyntax(
				`Expected '${String.fromCharCode(toBe)}' ${message}. Found '${thing !== undefined ? String.fromCharCode(thing) : 'undefined'}' instead`,
			)
		}
		if (advance) this.s.advance()
	}

	private expectSyntaxBoundaryAfter(message: string) {
		if (this.s.item && !CHARSETS.SYNTAX_BOUNDARY.includes(this.s.item)) {
			const previous = this.s.at(this.s.cursor - 1)
			if (previous && CHARSETS.SYNTAX_BOUNDARY.includes(previous)) return
	...
	private parseObject<T extends object, K extends keyof T & string = keyof T & string>({
		...
		if (this.currentNestingDepth > this.maxNestingDepth) {
			this.throwSyntax(`Exceeded maximum nesting depth of ${this.maxNestingDepth}`)
		}

		const startPosition = this.recordPosition()

		try {
			this.expect(this.s.item, CHARS.LEFT_CURLY_BRACKET, 'to begin ' + objectName, true)
			this.consumeWhitespace()
			const obj = {} as Partial<T>
			while (this.s.item !== CHARS.RIGHT_CURLY_BRACKET) {
				const keyPosition = this.recordPosition()
				const quoted = CHARSETS.QUOTES.includes(this.s.item!)
				...
				if (result) {
					...
					if (this.enabledFeatures & FEATURES.LITERAL_KEYS) {
						key = this.parseTextPrimitive() as K
					} else {
				...
				if (result?.value !== undefined) {
					...
					this.expect(this.s.item, CHARS.COLON, `to follow key '${key}'`, true)
					this.consumeWhitespace()
					...
					parseValue(key, obj)
					// If the value parser didn't advance anything, assume the value is missing.
				...
				if (this.s.item === CHARS.COMMA) {
					...
					if (
						this.s.item === CHARS.RIGHT_CURLY_BRACKET &&
						!(this.enabledFeatures & FEATURES.TRAILING_COMMAS)
					) {
						this.throwSyntax(`Trailing comma in ` + objectName, this.s)
					}
				} else if (this.s.item === CHARS.RIGHT_CURLY_BRACKET) {
					break
	...
	private parseArray<T extends any[]>({
		...
		this.expect(this.s.item, CHARS.LEFT_SQUARE_BRACKET, 'to begin ' + arrayName, true)
		this.consumeWhitespace()
		...
		while (this.s.item !== CHARS.RIGHT_SQUARE_BRACKET) {
			this.consumeWhitespace()
			...
			if (this.s.item === CHARS.COMMA) {
				...
				if (
					this.s.item === CHARS.RIGHT_SQUARE_BRACKET &&
					!(this.enabledFeatures & FEATURES.TRAILING_COMMAS)
				) {
					this.throwSyntax(`Trailing comma in ` + arrayName, this.s)
				}
			} else if (this.s.item === CHARS.RIGHT_SQUARE_BRACKET) {
				break
			} else if (this.s.item === undefined) {
	...
	private parseIntArray<T extends number[]>(requireTypeIdentifier?: boolean, length?: number): T {
		return this.parseArray<T>({
			arrayName: 'int-array',
			parsePrefix: () => {
				if (this.s.item === CHARS.I) {
					this.s.advance()
					this.consumeWhitespace()
					this.expect(
						this.s.item,
						CHARS.SEMICOLON,
						'to follow array type identifier',
						true
					)
	...
	private parseTextObject() {
		return this.parseObject<TextObject>({
			...
			parseKey: (keys, obj) => {
				...
				if (this.enabledFeatures & FEATURES.LITERAL_KEYS) {
					key = this.parseTextPrimitive() as keyof TextObject
				} else {
			...
			parseValue: (key, obj) => {
				switch (key) {
					...
					case 'translate':
					case 'fallback':
						obj[key] = this.parseTextPrimitive()
						break
					...
					case 'source':
						obj[key] = this.parseTextPrimitive(['storage', 'block', 'entity'])
						break
					...
					case 'object':
						...
						obj[key] = this.parseTextPrimitive(['atlas', 'player'])
						break
					...
					case 'type': {
						const type = this.parseTextPrimitive(Object.values(CONTENT_TYPES))
						if (
					...
					case 'clickEvent': {
						if (!(this.enabledFeatures & FEATURES.CLICK_EVENTS)) {
					...
					case 'hoverEvent': {
						if (!(this.enabledFeatures & FEATURES.HOVER_EVENTS)) {
	...
	private parseTextObjectShadowColor(): ShadowColor {
		...
		if (this.s.item === CHARS.LEFT_SQUARE_BRACKET) {
			...
			if (typeof color !== 'number') {
				this.throwSyntax(`Invalid number for 'shadow_color'`, this.s)
			}
			return TextComponent.intToRgba(color)
		}
		...
		let hexColor = this.parseTextPrimitive()
		if (hexColor.startsWith('#')) {
			hexColor = this.normalizeHex8Color(hexColor)
		} else if (hexColor in COLORS) {
			hexColor = COLORS[hexColor as keyof typeof COLORS]
		} else {
			this.throwSyntax(`Unknown color '${hexColor}'`, this.s)
		}
		return TextComponent.hexToRgba(hexColor)
	}
		...
		return this.parseObject<NonNullable<TextObject['player']>>({
			...
			parseValue: (key, obj) => {
				switch (key) {
					...
					case 'model':
						obj[key] = this.parseTextPrimitive(['wide', 'slim'])
						break
	...
	private parseLegacyClickEventObject() {
		return this.parseObject<LegacyClickEvent>({
			...
			parseValue: (key, obj) => {
				switch (key) {
					case 'action':
						const value = this.parseTextPrimitive([
							'open_url',
							'open_file',
							'run_command',
							'suggest_command',
		...
		return this.parseObject<MergeUnion<ModernClickEvent>>({
			...
			parseValue: (key, obj) => {
				switch (key) {
					...
					case 'url':
						const url: string = this.parseTextPrimitive()
						let parsedUrl: URL
						try {
	...
	private parseUnknownValue() {
		...
		} else if (CHARSETS.QUOTES.includes(this.s.item)) {
	...
	private parseBoolean(): boolean {
		if (this.s.peek(4).toLowerCase() === 'true') {
			this.s.advance()
			this.s.advance()
			this.s.advance()
			this.s.advance()
			return true
		} else if (this.s.peek(5).toLowerCase() === 'false') {
			this.s.advance()
			this.s.advance()
			this.s.advance()
			this.s.advance()
	...
	private parseNumber(type = NUMBER_TYPES.ANY): string {
		...
		let numberChars = CHARSETS.NUMBER

		const start = this.s.cursor
		let end = this.s.cursor

		const prefix = this.s.peek(2).toLowerCase()
		if (prefix === '0x') {
			this.s.advance()
			this.s.advance()
			end = this.s.cursor
			numberChars = CHARSETS.HEXADECIMAL
			type = NUMBER_TYPES.HEXADECIMAL
		} else if (prefix === '0b') {
			this.s.advance()
			this.s.advance()
			end = this.s.cursor
			numberChars = CHARSETS.BINARY
			type = NUMBER_TYPES.BINARY
		} else if (this.s.item === CHARS.MINUS) {
			this.s.advance()
			end = this.s.cursor
			this.consumeWhitespace()
		}
		...
		while (
			this.s.item &&
			(this.s.item === CHARS.PERIOD ||
				this.s.item === CHARS.UNDERSCORE ||
				numberChars.includes(this.s.item))
		) {
			this.parseDigits(type, numberChars)
			end = this.s.cursor
			this.consumeWhitespace()

			if (this.s.item === CHARS.E || this.s.item === CHARS.e) {
				if (!expectFloat) {
					this.throwSyntax(`E notation is not allowed in ${type}s`, this.s)
				}

				this.s.advance() // e or E
				end = this.s.cursor
				this.consumeWhitespace()

				const str = this.s.slice(start, end).replaceAll(`_`, ``)
			...
			} else if (this.s.item === CHARS.PERIOD) {
				if (hasDecimal) {
		...
		let str = this.s.slice(start, end).replaceAll(`_`, ``)
		let value: number
		switch (type) {
	...
	private parseHashedHexColor(): string {
		this.expect(this.s.item, CHARS.HASH, `to begin hex color`, true)

		const digits = this.parseDigits('hex', CHARSETS.HEXADECIMAL)
	...
	private parseDigits(
		...
		while (this.s.item != undefined && digitCharset.includes(this.s.item)) {
			...
			const beforeWhitespace = this.s.cursor
			const hasWhitespace = CHARSETS.WHITESPACE.includes(this.s.item)
			this.consumeWhitespace()

			if (this.s.item === CHARS.UNDERSCORE) {
				if (hasWhitespace) {
					this.throwSyntax(`Underscore must be between ${name} digits`, this.s)
				}
				while (this.s.item === CHARS.UNDERSCORE) {
					this.s.advance()
				}
				if (!digitCharset.includes(this.s.item)) {
					this.throwSyntax(`Underscore must be between ${name} digits`, this.s)
				}
			} else if (hasWhitespace) {
				// REVIEW - We should avoid backing up the stream like this.
				this.s.cursor = beforeWhitespace - 1
				this.s.advance()
				break
			}
		}
		if (this.s.cursor - start < minLength) {
			this.throwSyntax(`Not enough ${name} digits (min ${minLength})`, this.s)
		}

		return this.s.slice(start, this.s.cursor)
	}
	...
	/**
	 * Parses a JSON text primitive, which can be either a quoted string or an unquoted literal, depending on the enabled features.
	 */
	private parseTextPrimitive(): string
	private parseTextPrimitive<T>(validStringOptions: T[]): T
	private parseTextPrimitive<T extends string>(validStringOptions?: T[]): T {
		let str: T
		const startPosition = this.recordPosition()

		if (this.s.item && CHARSETS.QUOTES.includes(this.s.item)) {
	...
	private parseLiteral(): string {
		...
		if (!this.s.item || !CHARSETS.LITERAL_START.includes(this.s.item)) {
			this.throwSyntax(
	...
	private parseQuotedString(): string {
		...
		this.expect(this.s.item, CHARSETS.QUOTES, 'to begin string', true)

		if (
			this.s.item === CHARS.SINGLE_QUOTE &&
			!(this.enabledFeatures & FEATURES.SINGLE_QUOTES)
		) {
			this.throwSyntax(
		...
		while (this.s.item != undefined) {
			if (this.s.item === CHARS.BACKSLASH) {
				...
				break
			} else if (CHARSETS.VERTICAL_WHITESPACE.includes(this.s.item)) {
	...
	private parseEscapeSequence() {
		this.expect(this.s.item, CHARS.BACKSLASH, `to begin escape sequence`, true)

		const item = this.s.item
		if (
			item === CHARS.BACKSLASH ||
			item === CHARS.SINGLE_QUOTE ||
			item === CHARS.DOUBLE_QUOTE ||
			item === CHARS.n ||
			item === CHARS.b ||
			item === CHARS.r ||
			item === CHARS.t ||
			...
			this.s.advance()
		} else if (item === CHARS.u) {
			this.parseUnicodeEscapeSequence()
		} else if (item === CHARS.x) {
			...
			this.parseNamedUnicodeEscapeSequence()
		} else {
			this.throwSyntax(`Unknown escape sequence '\\${item!}'`, this.s)
		}
	}

	// Parses a string representation of the escape sequence.
	// The renderer, such as Minecraft, will handle the actual escape sequence resolution.
	private parseUnicodeEscapeSequence() {
		this.expect(this.s.item, CHARSETS.UNICODE_ESCAPE_SEQUENCE_START, `to begin escape sequence`)
		const expectedHexLength = this.s.item === CHARS.x ? 2 : this.s.item === CHARS.u ? 4 : 8
		this.s.advance() // u, x, or U
		this.parseHexDigits(expectedHexLength)
	}

	private parseHexDigits(count: number) {
		for (let i = 0; i < count; i++) {
			if (this.s.item && CHARSETS.HEXADECIMAL.includes(this.s.item)) {
				this.s.advance()
			} else {
				this.throwSyntax(
	...
	// Parses a string representation of the escape sequence.
	// The renderer, such as Minecraft, will handle the actual escape sequence resolution.
	private parseNamedUnicodeEscapeSequence() {
		if (this.s.peek(2) !== 'N{') {
			this.throwSyntax(
				`Expected 'N{' to begin named unicode escape sequence. Found '${this.s.peek(2)}' instead`
			)
		}
		const startIndex = this.s.cursor
		while (
			this.s.item &&
			this.s.item !== CHARS.RIGHT_CURLY_BRACKET &&
			!CHARSETS.VERTICAL_WHITESPACE.includes(this.s.item)
		) {
			this.s.advance()
		}
		const name = this.s.slice(startIndex, this.s.cursor).trim().toUpperCase()

		if (name.length === 0) {
			this.throwSyntax(`Expected character name in named unicode escape sequence`, this.s)
		}

		this.expect(
			this.s.item,
			CHARS.RIGHT_CURLY_BRACKET,
			`to end named unicode escape sequence`,
			true
		)
	}
}
