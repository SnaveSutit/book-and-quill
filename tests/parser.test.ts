...
function expectParsingResults(
	parser: TextComponentParser['parse'],
	map: InputOutputMap<string, TextElement>
) {
	for (const { input, expect: expectedResult } of map) {
		if (Array.isArray(input)) {
			for (const inputItem of input) {
				expect(parser(inputItem)).toEqual(expectedResult)
			}
		} else {
			expect(parser(input)).toEqual(expectedResult)
		}
	}
}
...
describe('TextComponentParser', () => {
	const modernParser = new TextComponentParser()
	const modernParse = modernParser.parse.bind(modernParser)
	const legacyParser = new TextComponentParser({ minecraftVersion: '1.20.4' })
	...
	describe('parses TextObjects', () => {
		describe('text field', () => {
			...
			test('implicit', () => {
				expect(modernParse(`{}`)).toEqual({ text: '' })
			})
		})
		...
		describe('color field', () => {
			test('named colors', () => {
				for (const color of Object.keys(COLORS)) {
					...
					const expected = { color: color as keyof typeof COLORS, text: '' }
			...
			expectParsingResults(modernParse, [
				{
					...
					expect: { shadow_color: TextComponent.hexToRgba('#ff00aced'), text: '' },
				},
				{
				...
				{
					input: `{shadow_color:[0,0.6745098039215687,0.9294117647058824,1]}`,
					expect: { shadow_color: TextComponent.hexToRgba('#ff00aced'), text: '' },
				},
				{
					input: `{shadow_color:-16732947}`,
					expect: {
						shadow_color: TextComponent.hexToRgba('#ff00aced'),
						text: '',
					},
				},
			])
			...
			for (const field of fields) {
				for (const { value, variants } of cases) {
					for (const variant of variants) {
						...
						const expected = { [field]: value, text: '' }
						expect(modernParse(input)).toEqual(expected)
					}
				}
			}
		...
		test('modern click_event field', () => {
			expectParsingResults(modernParse, [
				...
				{
					input: `{click_event:{action:'custom',id:'my_custom_action',payload:{count:1,nested:[2,{value:3}]}}}`,
					expect: {
						click_event: {
							action: 'custom',
							id: 'my_custom_action',
							payload: { count: 1, nested: [2, { value: 3 }] },
						},
						text: '',
					},
				},
		...
		test('modern hover_event field', () => {
			expectParsingResults(modernParse, [
				...
				{
					input: `{hover_event:{action:'show_item',id:'minecraft:stone',count:1,components:{level:2,nested:{enabled:true,weight:3}}}}`,
					expect: {
						hover_event: {
							action: 'show_item',
							id: 'minecraft:stone',
							count: 1,
							components: { level: 2, nested: { enabled: true, weight: 3 } },
						},
						text: '',
					},
				},
